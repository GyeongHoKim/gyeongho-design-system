...
- Add a comment explaining any new `sys`-tier role introduced for a documented one-off variant.

**Don't**
- Hardcode a hex value, pixel value, or numeric roughness — or silently break the `comp → sys → ref` chain by having a `comp` token reference `ref` directly.
