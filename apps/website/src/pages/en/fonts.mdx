...
and `안녕하세요` in **Pretendard** automatically.
...
| Body / UI (sans) | Nunito Sans Variable | Pretendard | Warm, legible, trustworthy |
...
- **Pretendard** — the de-facto Korean sans in Korean product design, designed as a drop-in
  replacement for system fonts with excellent Korean legibility and a neutral, trustworthy tone.

> **Alternatives.** If your product needs a smaller payload, `@fontsource-variable/noto-sans-kr`
> (a single variable file) can replace Pretendard. If a more neutral Latin face is preferred, Inter
...
| Pretendard (body, Korean) | `@fontsource/pretendard` | 400 / 500 / 700 |
...
in `@ghds/tokens`. Nunito Sans Variable is a single variable file covering all weights.
...
- **Pretendard** is a static font (no variable version on Fontsource). Import only the weights you
  use — 400 / 500 / 700 covers body, label, and heading weights. Each weight is ~70–90 KB (woff2,
  subset for Korean is larger; consider loading 400 first and 500/700 async if payload is critical).
...
[Pretendard releases](https://github.com/orioncactus/pretendard/releases) and
